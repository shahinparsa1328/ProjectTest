...
export const FamilyPage: React.FC<FamilyPageProps> = ({ 
  ...
  // --- Phase 3.3 & 3.5 AI States ---
  const [aiFamilyMemoryReminders, setAiFamilyMemoryReminders] = useState<FamilyMemoryReminder[]>([]);
  const [isLoadingMemoryReminders, setIsLoadingMemoryReminders] = useState(false);
  const [memoryRemindersError, setMemoryRemindersError] = useState<string | null>(null);
  
  const [aiIntegrationSuggestions, setAiIntegrationSuggestions] = useState<AISuggestionFamily[]>([]);
  const [isLoadingIntegrationSuggestions, setIsLoadingIntegrationSuggestions] = useState(false);
  const [integrationSuggestionsError, setIntegrationSuggestionsError] = useState<string | null>(null);

  // Static data (can be moved to constants if preferred)
  ...
  const simulateNewElderlySensorData = () => {
   ...
   // --- Phase 3.3 & 3.5 AI Fetch Functions ---
  const fetchAIMemoryReminders = useCallback(async () => {
    if (!geminiAI) { setMemoryRemindersError(toPersianDigits("سرویس AI در دسترس نیست.")); return; }
    setIsLoadingMemoryReminders(true); setMemoryRemindersError(null);
    try {
      const pastEventsSample = calendarEventsData.filter(e => new Date(e.start) < new Date()).slice(0,3).map(e => e.title).join('، ');
      const albumsSample = photoAlbumsData.slice(0,2).map(a => a.name).join('، ');
      const prompt = toPersianDigits(`با توجه به رویدادهای گذشته تقویم خانواده (مانند "${pastEventsSample || 'جشن تولد'}") و آلبوم‌های عکس (مانند "${albumsSample || 'تعطیلات تابستان'}"), ۱ تا ۲ یادآوری خاطره‌انگیز و دلنشین به زبان فارسی برای خانواده تولید کن. هر یادآوری باید شامل "title", "description" (متن یادآوری), "memoryDate" (تاریخ خاطره به فرمت YYYY-MM-DD), "sourceType" ('calendar' یا 'album') و "xaiRationale" (توضیح دلیل پیشنهاد این یادآوری) باشد. پاسخ باید یک آرایه JSON باشد.`);
      const response: GenerateContentResponse = await geminiAI.models.generateContent({ model: AI_MODEL_NAME_FAMILY, contents: prompt, config: { responseMimeType: "application/json" }});
      const parsedData = parseJsonFromString<Omit<FamilyMemoryReminder, 'id'>[]>(response.text);
      if (parsedData) { 
        setAiFamilyMemoryReminders(parsedData.map((rem, idx) => ({...rem, id: `ai-mem-${Date.now()}-${idx}`}))); 
      } else { throw new Error("پاسخ نامعتبر برای یادآوری خاطرات.");}
    } catch (e: any) { setMemoryRemindersError(toPersianDigits("خطا در دریافت یادآوری خاطرات.")); console.error(e); } 
    finally { setIsLoadingMemoryReminders(false); }
  }, [geminiAI, calendarEventsData, photoAlbumsData]);

  const fetchAIIntegrationSuggestions = useCallback(async () => {
    if (!geminiAI) { setIntegrationSuggestionsError(toPersianDigits("سرویس AI در دسترس نیست.")); return; }
    setIsLoadingIntegrationSuggestions(true); setIntegrationSuggestionsError(null);
    try {
        const childAges = familyMembersData.filter(m => m.role === 'فرزند' && m.dob).map(m => new Date().getFullYear() - new Date(m.dob).getFullYear()).join(', ') || 'کودک نوپا';
        const elderNeeds = careRemindersData.length > 0 ? 'نیاز به مراقبت از سالمند دارد' : 'نیاز خاصی برای سالمند ثبت نشده';
        const prompt = toPersianDigits(`یک خانواده با کودکان در سنین (${childAges}) و وضعیت مراقبت از سالمند (${elderNeeds}) وجود دارد. از لیست پلتفرم‌های آموزشی (مانند "آکادمی والدین هوشمند") و دسته‌بندی‌های خدمات مراقبتی (مانند "خدمات مراقبت از کودک")، ۱ تا ۲ پیشنهاد مرتبط و مفید برای این خانواده به زبان فارسی ارائه بده. هر پیشنهاد باید شامل "title"، "text" (توضیح پیشنهاد و دلیل مفید بودن آن)، "xaiRationale" (دلیل عمیق‌تر انتخاب این پیشنهاد برای این خانواده) و "type" ('integration_suggestion') باشد. پاسخ باید یک آرایه JSON باشد.`);
        const response: GenerateContentResponse = await geminiAI.models.generateContent({ model: AI_MODEL_NAME_FAMILY, contents: prompt, config: { responseMimeType: "application/json" }});
        const parsedData = parseJsonFromString<AISuggestionFamily[]>(response.text);
        if (parsedData) { 
            setAiIntegrationSuggestions(parsedData.map(sugg => ({...sugg, id: `ai-integ-${Date.now()}-${Math.random()}`}))); 
        } else { throw new Error("پاسخ نامعتبر برای پیشنهادات یکپارچه‌سازی.");}
    } catch (e: any) { setIntegrationSuggestionsError(toPersianDigits("خطا در دریافت پیشنهادات یکپارچه‌سازی.")); console.error(e); } 
    finally { setIsLoadingIntegrationSuggestions(false); }
  }, [geminiAI, familyMembersData, careRemindersData]);
  ...
  useEffect(() => {
    ...
    setElderlyHealthData(loadFromLocalStorage(LS_ELDERLY_HEALTH_DATA, [ 
    ...
    fetchAIMemoryReminders(); 
    fetchAIIntegrationSuggestions(); 
    ...
    <div className={`page ${activeTheme.backgroundClass} pb-8`}>
      ...
      <CollapsibleSection title={toPersianDigits("رشد و مراقبت از کودک")} icon={<AcademicCapIcon className={`w-5 h-5 ${activeTheme.primaryAccentClass}`} />} isOpen={openSections.childDevelopment} onToggle={() => toggleSection('childDevelopment')} className={`mb-5 bg-white p-4 rounded-xl shadow-sm border ${activeTheme.secondaryAccentClass}`} titleColorClass={`text-md font-semibold ${activeTheme.primaryAccentClass}`} >
         <ChildDevelopmentSection 
            ...
            parentingTips={dynamicParentingTips} // Pass dynamic tips
            gameIdeas={gameIdeas} 
            ...
            primaryAccentClass={activeTheme.primaryAccentClass} 
            childHealthAlerts={childHealthAlerts} // Pass alerts
      ...
      <CollapsibleSection title={toPersianDigits("مراقبت و پشتیبانی از سالمند")} icon={<ShieldCheckIcon className={`w-5 h-5 ${activeTheme.primaryAccentClass}`} />} isOpen={openSections.elderlyCare} onToggle={() => toggleSection('elderlyCare')} className={`mb-5 bg-white p-4 rounded-xl shadow-sm border ${activeTheme.secondaryAccentClass}`} titleColorClass={`text-md font-semibold ${activeTheme.primaryAccentClass}`} >
        <ElderlyCareSection 
          ...
          aiSuggestions={dynamicElderlyCareSuggestions} // Pass dynamic suggestions
          ...
          onRefreshAIAnalysis={simulateNewElderlySensorData} // This will also trigger fetch
        />
      </CollapsibleSection>

      {/* Memory Reminders Section (Phase 3.3) */}
        <CollapsibleSection 
            title={toPersianDigits("یادآوری‌های خاطرات با AI")} 
            icon={<QualityTimeIcon className={`w-5 h-5 ${activeTheme.primaryAccentClass}`} />} 
            isOpen={openSections.memoryReminders} 
            onToggle={() => toggleSection('memoryReminders')} 
            className={`mb-5 bg-white p-4 rounded-xl shadow-sm border ${activeTheme.secondaryAccentClass}`} 
            titleColorClass={`text-md font-semibold ${activeTheme.primaryAccentClass}`}
        >
            <div className="flex justify-end mb-2">
                <button onClick={fetchAIMemoryReminders} disabled={isLoadingMemoryReminders || !geminiAI} className={`p-1 rounded-full hover:bg-opacity-20 ${activeTheme.buttonHoverBgClass.replace('hover:bg-','')} disabled:opacity-50`} title={toPersianDigits("بارگذاری مجدد یادآوری‌ها")}><ArrowPathIcon className="w-3.5 h-3.5"/></button>
            </div>
            {isLoadingMemoryReminders ? <LoadingSpinner/> : memoryRemindersError ? <p className="text-xs text-red-500">{memoryRemindersError}</p> :
             aiFamilyMemoryReminders.length > 0 ? (
                <div className="space-y-3">
                    {aiFamilyMemoryReminders.map(reminder => (
                        <FamilyMemoryReminderCard key={reminder.id} reminder={reminder} themeClasses={activeTheme} onShowXai={(rationale) => handleShowXAIInternal(toPersianDigits("منطق یادآوری"), rationale)} />
                    ))}
                </div>
            ) : (<p className="text-xs text-gray-500 text-center">{toPersianDigits("در حال حاضر یادآوری خاطره جدیدی از AI وجود ندارد.")}</p>)}
        </CollapsibleSection>
      ...
      {/* External Integrations Section (Phase 3.5) */}
        <CollapsibleSection 
            title={toPersianDigits("یکپارچه‌سازی‌های هوشمند")} 
            icon={<IntegrationsIcon className={`w-5 h-5 ${activeTheme.primaryAccentClass}`} />} 
        ...
        >
            <ExternalIntegrationsSection 
                ...
                careServiceCategories={careServiceCategories} 
                aiSuggestions={aiIntegrationSuggestions} 
                themeClasses={activeTheme} 
                onShowXai={(title, rationale) => handleShowXAIInternal(title, rationale)}
                geminiAI={geminiAI}
                familyMembers={familyMembersData}
                showToast={showToast}
            />
